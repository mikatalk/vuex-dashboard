...
const fragment = `
  ...
  void main() {
    ...
    // repeat
    uv = repeat(uv, vec2(1., 1.));
    
    float ratio = (1.0 + sin(elapsedTime))/ 2.0;

    // float scale = ratio;
    //(1.0 + sin(ratio * PI * 2.0 + length(vec2(1.0)/length(vUv-vec2(.5)))) )/ 2.0;
    ...
    uv = rotate(uv, -length(vec2(.5) - vUv) * 5. * PI + elapsedTime * 3.1);
    alpha = (sdHexPrism(vec3(0.1, 1.9, .4), uv));
